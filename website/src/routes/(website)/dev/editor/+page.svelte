        ...
        <div class="flex-1 overflow-y-auto p-1.5 min-h-0 space-y-0.5 scrollbar-thin">
          ...
          {:else}
            {#each filtered as file}
              <button onclick={() => loadChapter(file)} title="{indices.has(file) ? 'ch' + (currentBook === "debut" ? Number(indices.get(file)) + 1 : indices.get(file)) : file}{titles.has(file) ? ' - ' + titles.get(file) : ''}{dirty.has(file) ? ' (modified)' : ''}" class="block w-full text-left text-xs px-2.5 py-1.5 rounded-lg hover:bg-base-content/5 transition-colors whitespace-nowrap overflow-hidden text-ellipsis {selected === file ? 'bg-primary/10 text-base-content' : dirty.has(file) ? 'text-success' : 'text-base-content/70'}">
      ...
      <div class="w-64 flex flex-col bg-base-200/40 rounded-xl border border-base-content/10 shrink-0 min-h-0">
        ...
        <div class="flex-1 overflow-y-auto overflow-x-hidden scrollbar-thin">
          <table class="w-full border-collapse">
            <tbody>
              {#each [
                { syntax: "%%text%%", desc: "Shake effect (block)" }, { syntax: "%~text~%", desc: "Shake effect (per-char)" }, { syntax: "%^text^%", desc: "Wave up effect" }, { syntax: "@@text@@", desc: "Glitch text (heavy)" }, { syntax: "@_@text@_@", desc: "Glitch text (subtle)" }, { syntax: "#^#text#^#", desc: "Grow font size" }, { syntax: "#v#text#v#", desc: "Shrink font size" }, { syntax: "~~~", desc: "Visible horizontal rule" }, { syntax: "~^~", desc: "Invisible section break" }, { syntax: "_text_", desc: "Underline" }, { syntax: "~~text~~", desc: "Strikethrough" }, { syntax: "@ll@text@ll@", desc: "Mono left-aligned" }, { syntax: "@rr@text@rr@", desc: "Mono right-aligned" }, { syntax: "@l@text@l@", desc: "Left align" }, { syntax: "@r@text@r@", desc: "Right align" }, { syntax: "#*text*#", desc: "Large text" }, { syntax: "#><text><#", desc: "Large centered text" }, { syntax: "#rtextr#", desc: "Red text" }, { syntax: "#btextb#", desc: "Blue text" }, { syntax: "#ytexty#", desc: "Yellow text" }, { syntax: "#ptextp#", desc: "Magenta text" }, { syntax: "#gtextg#", desc: "Green text" }, { syntax: "#otexto#", desc: "Orange text" }, { syntax: "#lptextlp#", desc: "Purple text" }, { syntax: "#cytextcy#", desc: "Cyan text" }, { syntax: "$agtextag$", desc: "Silver text" }, { syntax: "#f#text#f#", desc: "Fade out" }, { syntax: "#f#>text>#f#", desc: "Fade right" }, { syntax: "#f#<text<#f#", desc: "Fade left" }, { syntax: "$stexts$", desc: "Smoke text" }, { syntax: "$atexta$", desc: "Aurora text" }, { syntax: "$gtextg$", desc: "Gold text" }, { syntax: "$*text*$", desc: "Sparkle text" }, { syntax: "$(text)$", desc: "Moon text" }, { syntax: "-# text #-", desc: "Sub/small text" }, { syntax: ";rtextr;", desc: "Red highlight" }, { syntax: ";btextb;", desc: "Blue highlight" }, { syntax: ";ytexty;", desc: "Yellow highlight" }, { syntax: ";ptextp;", desc: "Magenta highlight" }, { syntax: ";gtextg;", desc: "Green highlight" }, { syntax: ";otexto;", desc: "Orange highlight" }, { syntax: "+-text-+", desc: "Wiki window" }, { syntax: "+$text$+", desc: "Plain window" }, { syntax: "&$text$&", desc: "Followup window" }, { syntax: "&--text--&", desc: "Record window" }, { syntax: "+~text~+", desc: "System window" }, { syntax: "+=text=+", desc: "Black CRT window" }, { syntax: "!-text-!", desc: "Notepad window" }, { syntax: "!$text$!", desc: "Sticky note window" }, { syntax: "![text]!", desc: "Braun CRT monitor" }, { syntax: "★-text-★", desc: "DoD window" }, { syntax: "★!text!★", desc: "DoD alert" }, { syntax: "★=text=★", desc: "DoD achievement" }, { syntax: "★$text$★", desc: "Comment window" }, { syntax: "★:text:★", desc: "SMS window" }, { syntax: "}text}", desc: "Oriented sub (left)" }, { syntax: "{text{", desc: "Oriented sub (right)" },
              ] as opt}
        ...
        <div class="flex-1 min-h-0 px-3 pb-3 pt-1.5">
          ...
          <div class="h-full flex flex-col bg-base-200/80 backdrop-blur-sm rounded-xl border border-base-content/10 shadow-lg">
            ...
            <div class="flex-1 overflow-y-auto p-1.5 min-h-0 space-y-0.5 scrollbar-thin">
              ...
              {:else}
                {#each filtered as file}
                  <button onclick={() => { loadChapter(file); showMobileMenu = false; }} title="{indices.has(file) ? 'ch' + (currentBook === "debut" ? Number(indices.get(file)) + 1 : indices.get(file)) : file}{titles.has(file) ? ' - ' + titles.get(file) : ''}{dirty.has(file) ? ' (modified)' : ''}" class="block w-full text-left text-xs px-2.5 py-1.5 rounded-lg hover:bg-base-content/5 transition-colors whitespace-nowrap overflow-hidden text-ellipsis {selected === file ? 'bg-primary/10 text-base-content' : dirty.has(file) ? 'text-success' : 'text-base-content/70'}">
                    {#if indices.has(file)}<span class="font-medium">ch{currentBook === "debut" ? Number(indices.get(file)) + 1 : indices.get(file)}</span>{:else}<span class="font-medium">{file.replace('.md','')}</span>{/if}
                    {#if titles.has(file)}<span class="text-base-content/50 ml-1">— {titles.get(file)}</span>{/if}
                    {#if dirty.has(file)}<span class="text-success/60 ml-1">●</span>{/if}
                  </button>
